...
test('every category has a hook to start with, inside its own range', () => {
	for (const c of CATEGORIES) {
		assert.ok(c.hook.from < c.hook.to, `category ${c.n}: range should run low to high`);
		assert.ok(c.hook.start >= c.hook.from && c.hook.start <= c.hook.to, `category ${c.n}: start outside range`);
	}
	// The ranges are CYC's, checked here so a slip in the table fails rather than ships.
	assert.deepEqual([cat(3).hook.from, cat(3).hook.to], [4.5, 5.5]);
	assert.deepEqual([cat(6).hook.from, cat(6).hook.to], [9, 15]);
});
...
test('the hook reads as a size to start with, then the range', () => {
	...
	assert.equal(answer(form({ m100: '250' })).category.hook.start, 5);
});
...
test('the length and the blend are judged separately', () => {
	// One refusal from calculate used to be the page's only source, and a wrong length hid a
	// wrong blend. Each can now be asked for on its own.
	const input = form({ m100: '900000', rows: [{ fibre: 'Wool', pct: '96' }] });
	assert.match(lengthProblem(input)!, /is not a length any yarn has/);
	assert.equal(blendProblem(input.rows), MESSAGES.needsTotal);
	assert.equal(lengthProblem(form({ m100: '250' })), null);
	assert.equal(blendProblem([{ fibre: 'Wool', pct: '' }]), null);
	assert.equal(blendProblem([{ fibre: '', pct: '' }]), MESSAGES.needsBlend);
	assert.equal(blendProblem([{ fibre: '', pct: '40' }, { fibre: 'Wool', pct: '60' }]), MESSAGES.needsFibre);
});

test('a figure finer than any yarn is answered, with a note', () => {
	const r = answer(form({ m100: '25000' }));
	assert.equal(r.category.n, 0);
	assert.equal(r.check?.tone, 'caution');
	assert.match(r.check!.message, /^25,000 m\/100 g is finer than almost any knitting or crochet yarn/);
	// Real lace passes without one.
	assert.equal(answer(form({ m100: '2400' })).check, null);
});

test('a count with no length says how to check it by hand', () => {
	assert.match(answer(form({ millB: '2500' })).check!.message, /measure out 10 m and weigh it/);
});
...
test('one strand already heavier than the target', () => {
	assert.equal(said(strandsToReach(120, cat(0))), 'None — already heavier | One strand is already heavier than 0 Lace.');
});
