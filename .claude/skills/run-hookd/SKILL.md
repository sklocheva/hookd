...
- **Pick a marker that did not exist before.** Not a driver issue, but the recurring mistake
  on this project: verifying a change with a string that was already present (or that the
  minifier never emits) reports success or failure that means nothing. Check something the
  change actually creates.
